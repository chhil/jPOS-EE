...
public class BranchFieldPackager extends AFSDFieldPackager {
    ...
    public String getParserTree(String prefix) {
        ...
        if (switchCases == null) {
            ...
            cases += prefix+"[Not Set]" + System.lineSeparator();
        } else {
            for (Map.Entry<String, AFSDFieldPackager> entry : switchCases.entrySet()) {

                AFSDFieldPackager fPkgr = entry.getValue();

                cases += prefix+"\t\t" + entry.getKey() + ":" + System.lineSeparator() + fPkgr.getParserTree(prefix+"\t\t\t");
            }
        }
        cases += prefix+"\t\tdefault:" + System.lineSeparator() + ((defaultCase != null)
                ? System.lineSeparator() + defaultCase.getParserTree(prefix+"\t\t\t") : prefix+"\t\t\t[Not Set]");

        return String.format("%sField [%s] : [Branch]%n" + prefix+"\tswitch (%s)%n" + "%s", prefix, getName(), switchField,
                cases);
    }
